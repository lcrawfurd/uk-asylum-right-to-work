...
It prints every number and writes [`numbers.json`](numbers.json). `make_barchart.py`, `make_figure.py`, and `make_interactive.py` regenerate the figures from the same model. Change an assumption (a named constant at the top of `model.py`) and re-run to see how every number moves. This is a **local sandbox only** — it updates the console output and `numbers.json` in this repo, and is not connected to the published blog or its figures.
...
> **Figures.** The chart above is the headline. This repo also shows the individual view — the [40-year trajectory](#one-worker-over-40-years) below — as a static figure and an [animated, interactive version](BOTEC-asylum-figure3.html).
...
- **`make_barchart.py`** → **`figure-charge-vs-righttowork.svg`** — the headline figure (charge vs the two right-to-work channels), regenerated from `numbers.json`.
- **`make_figure.py`** → **`figure-one-worker.svg`** — the static two-panel individual trajectory.
- **`make_interactive.py`** → **`BOTEC-asylum-figure3.html`** — the trajectory as an *animated, interactive* panel (step through the scenarios, hover to read each year; built to the [CGD Interactive Toolkit](https://github.com/Center-for-Global-Development/cgd-interactive-toolkit); see `TRACKING.md`).
- All three generators derive every plotted coordinate from the model — no hand-drawing.
