...
> **Figures.** The charts live in the blog. Kept in this repo: the animated 20-year individual trajectory, [`BOTEC-asylum-figure3.html`](BOTEC-asylum-figure3.html) (shown [below](#one-worker-over-20-years)).
...
The aggregate figures net out across everyone. It helps to see the logic for a single person who finds work — the status quo (A), the charge bolted on (B), and the charge paired with the right to work at three months (C).
...
*Animated, interactive version: [`BOTEC-asylum-figure3.html`](BOTEC-asylum-figure3.html).*

Three things it shows: **the charge changes nothing about earnings** (A and B are identical lines); **even from a worker it barely collects** (£3,312 of £10,000 after twenty years); and **the right to work pays the Exchequer far more through tax than the charge ever recovers** — ~£45k of extra income tax and NI, more than four times the charge, and the only scenario in which the charge is actually cleared. The non-linearity cuts further against the policy as framed: if the first months of a ban do most of the damage, six months is a compromise, not a cure.
...
- **`model.py`** — the whole model in one runnable file (Channel A amortisation, the 20-year trajectory, the earner distribution, Channels B & C, hotel arithmetic). Sources noted inline.
- **`numbers.json`** — generated output; every headline number.
- **`figure-one-worker.svg`** — the trajectory chart shown above.
- **`BOTEC-asylum-figure3.html`** — its animated, interactive version (built to the [CGD Interactive Toolkit](https://github.com/Center-for-Global-Development/cgd-interactive-toolkit); see `TRACKING.md`).
