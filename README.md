...
It prints every number and writes [`numbers.json`](numbers.json). `python3 make_figure.py` and `python3 make_interactive.py` regenerate the static and interactive figures from the same model. Change an assumption (a named constant at the top of `model.py`) and re-run to see how every number moves. This is a **local sandbox only** — it updates the console output and `numbers.json` in this repo, and is not connected to the published blog or its figures.
...
> **Figures.** The charts live in the blog. This repo shows one — the [40-year individual trajectory](#one-worker-over-40-years) below — also as an animated, interactive version in [`BOTEC-asylum-figure3.html`](BOTEC-asylum-figure3.html).
...
- **`make_figure.py`** / **`make_interactive.py`** — regenerate the two figures from `model.py`, so every plotted coordinate is exact.
- **`figure-one-worker.svg`** — the static trajectory chart shown above (generated by `make_figure.py`).
- **`BOTEC-asylum-figure3.html`** — the same figure as an *animated, interactive* single panel: step through the three scenarios, hover to read each year (generated by `make_interactive.py`; built to the [CGD Interactive Toolkit](https://github.com/Center-for-Global-Development/cgd-interactive-toolkit); see `TRACKING.md`).
